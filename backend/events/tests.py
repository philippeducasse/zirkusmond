...
def make_show(**kwargs):
    defaults = dict(title='Test Show', description='', cast='', card_image=make_image(), private=False, base_ticket_price=15)
...
class ShowPriceValidationTest(TestCase):
    def setUp(self):
        self.show_data = dict(
            title='Test Show', description='A description', cast='A cast',
            card_image=make_image(), private=False,
        )

    def test_valid_price_config_passes(self):
        show = Show(**self.show_data, base_ticket_price=20, min_ticket_price=10, max_ticket_price=30)
        show.full_clean()
